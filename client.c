...
serv_addr.sin_addr.s_addr=INADDR_ANY;//inet_addr("10.42.0.127");

serv_addr.sin_port=htons(PORT);

result=connect(sockfd,(struct sockaddr *)&serv_addr,sizeof(serv_addr));

if(result<0){ perror("not connected"); exit(1);  }

printf("Connection accepted !!! ;) \n ");

    pthread_create(&thread1,NULL,print_receive_msg,(void *)sockfd);
    ...
    if(strcmp(buffer_write,QUIT)==0){write(sockfd,buffer_write,strlen(buffer_write));break;}
